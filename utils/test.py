...
from utils.svd_lm import AutoSVDHandler
...
def run_wikitext():
    ...
    selected_dataset = train_dataset.shuffle(seed=42).select(range(8))
    ...
    grad_profiles = svd_handler.profile_grad(return_grad_val=True)
    ...
    visualizer.grad_plot(grad_profiles, show=False, save=True)
